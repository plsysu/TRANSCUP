...
if (is.null(opt$data)){
  ...
  stop("The FPKM matrix must be supplied, rows are genes columns are samples, sep by tab.", call.=FALSE)
}
...
# read FPKM table
fpkm_df <- read.table(FPKM_file,header = T,sep = "\t",stringsAsFactors = F,row.names = 1)
fpkm_df <- t(fpkm_df)

test_df_select_feature <- fpkm_df[,selected_features]
